...
const EditPersonModal = ({ isOpen, onClose, onUpdate, person, category, researchAreas }) => {
  ...
  useEffect(() => {
    if (person) {
      setFormData({
        ...
        googleScholarProfile: person.googleScholarProfile || '',
        researchGateProfile: person.researchGateProfile || '',
        biography: person.biography || '',
        achievements: person.achievements && person.achievements.length > 0 ? person.achievements : ['']
      });
    }
  }, [person]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await onUpdate(person.id, cleanedData, category);
      onClose();
    } catch (error) {
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 p-6 flex items-center justify-between rounded-t-xl z-10">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">Update person information</p>
            </div>
          </div>
          <Button
            variant="ghost"
